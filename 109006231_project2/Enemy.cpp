...
#include "Turret.hpp"
#include "Collider.hpp" //for block

PlayScene* Enemy::getPlayScene() {
	return dynamic_cast<PlayScene*>(Engine::GameEngine::GetInstance().GetActiveScene());
}
void Enemy::OnExplode() {
	//explode, then dirty dust ash afterwards
...
Enemy::Enemy(std::string img, float x, float y, float radius, float speed, float hp, int money, int coolDown) : //new
...
void Enemy::Update(float deltaTime) {
	...
	int flag = 1;
	for (auto& c : scene->TowerGroup->GetObjects()) {
		Turret* turret = dynamic_cast<Turret*>(c);
		if (turret->GetPrice() == 70 || !turret->Visible) continue;
	...
	if (flag == 0) {
		...
		Position.x -= Velocity.x * deltaTime/11;
	...
	if(Position.x <= PlayScene::EndGridPointx * PlayScene::BlockSize + PlayScene::BlockSize / 2){
		Hit(hp); //enemy goes to baseline enemy dies
		getPlayScene()->Hit();  //game lives--
		reachEndTime = 0;
		return;
	}
