...
void PlayScene::OnMouseUp(int button, int mx, int my) {
	...
	if (button & 1) {
		...
		//	//
			...
			for (auto& c : TowerGroup->GetObjects()) {
				Turret* turret = dynamic_cast<Turret*>(c);
				if (turret->Position == preview->Position) {
					turret->TurretHit(20);
					break;
				}
			}
...
void PlayScene::OnKeyDown(int keyCode) {
	...
	// TODO 2 (5/8): Make the E key to create the 3th turret.
		...
		//hotkey for DummyTurret
		UIBtnClicked(3);
	}
	else if (keyCode == ALLEGRO_KEY_T) {
		//hotkey for BombTurret
		UIBtnClicked(4);
	}
	else if (keyCode == ALLEGRO_KEY_Y) {
		//hotkey for CancelTurret
		UIBtnClicked(5);
	}
...
void PlayScene::ConstructUI() {
	...
	//CancelTurret Btn
	btn = new TurretButton("play/floor.png", "play/dirt.png",
		Engine::Sprite("play/target-invalid.png", 780, BlockSize * MapHeight, 0, 0, 0, 0)
		, 770, 128 * MapHeight, CancelTurret::Price);
...
void PlayScene::UIBtnClicked(int id) {
	...
	else if (id == 3 && money >= DummyTurret::Price)
		preview = new DummyTurret(0, 0);
	else if (id == 4 && money >= BombTurret::Price)
		preview = new BombTurret(0, 0);
	else if (id == 5 && money >= CancelTurret::Price)
		preview = new CancelTurret(0, 0);
	if (!preview)
		return;
	preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
...
void PlayScene::ResetMapState(int x, int y) {
	const int new_x = (const int)(x / BlockSize);
	const int new_y = (const int)(y / BlockSize);
	mapState[new_y][new_x] = TILE_CLEAR;
	//mapState[y/128][x/128] = TILE_CLEAR;
}
